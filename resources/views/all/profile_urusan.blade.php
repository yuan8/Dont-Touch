			...
			<div class="card-body listing-menu" style="max-height: 80vh; overflow-y:scroll;">
				<ul class="todo-list-wrapper list-group list-group-flush">
					<?php foreach ($data_head['data'] as $key => $value): ?>
						  <li href="#" onclick="get_chart(this,'bu');" data-toggle="collapse" dt="{{$key}}" data-target="#mn-{{$key}}" class="list-group-item list-group-item-action border-btm-card border-primary" style="font-size: 10px;">
							<span class="badge badge-pill badge-primary">bu</span> 

						    {{$value['nama']}}
						  </li>
						<div id="mn-{{$key}}" class="collapse drop-menu" >
							<ul class="todo-list-wrapper list-group list-group-flush">
								<?php foreach ($value['daerah'] as $k => $d): ?>
										<li href="#" onclick="get_chart(this,'d')" data-toggle="collapse" dt="{{$key}}|daerah|{{$k}}" data-target="#mn-{{$key}}-{{$k}}" class="list-group-item list-group-item-action border-btm-card border-success " style="font-size: 10px;">
											 <span class="badge badge-pill badge-success">d</span> 
										    {{$d['nama']}}
										  </li>
										  <div id="mn-{{$key}}-{{$k}}" class="collapse drop-menu" >
												<ul class="todo-list-wrapper list-group list-group-flush">
													<?php foreach ($d['program'] as $s => $sb): ?>
															
															<li href="#" onclick="get_chart(this,'p')" dt="{{$key}}|daerah|{{$k}}|program|{{$s}}" class="list-group-item list-group-item-action  border-btm-card border-warning" style="font-size: 10px;">
															   <span class="badge badge-pill badge-warning">p</span> {{$sb['nama']}}
															  </li>
...
<script type="text/javascript">
	...
	function get_chart(dom,tag){
		...
		if((tag=='p')){
			$('#chart').html('');
			var data_send={
			"id_urusan":parseInt(key[0]),
			"kode_daerah":(""+key[2]).toString(),
			"kode_program":""+(key[4]).toString()
			}
