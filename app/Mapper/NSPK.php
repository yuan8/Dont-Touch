...
class NSPK extends Model
{
	...
	public static function query($tahun=2020){
		$data=DB::select(
			"select s.nama as nama_sub_urusan,count(case when sdgs then 1 end ) as jml_sdgs,count(case when pn then 1 end ) as jml_pn,count(case when spm then 1 end ) as jml_spm,count(case when nspk then 1 end ) as jml_nspk,sum(anggaran) as jml_anggaran,kode_daerah,d.nama as nama_daerah,a.id_urusan , m.id as id_mandat, m.mandat as mandat,u.nama as nama_urusan,a.id_sub_urusan,kode_program,uraian_kode_program_daerah, count(DISTINCT(kode_program)) as jml_program,count(DISTINCT(a.kode_kegiatan)) as jml_kegiatan from 
            ...
            where a.tahun =
            ".$tahun." and a.id_nspk != 0 and a.nspk=true
