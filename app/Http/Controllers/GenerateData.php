...
class GenerateData extends Controller
{
    ...
    public function urusan_23(){

    	$datas=DB::connection('pgsql2')->table('ms_urusan_23')->get();


    	$datas=json_decode($datas,true);

    	$tbd=DB::connection('pgsql2')->table('master_urusan')->truncate();
    	$tbd=DB::table('master_urusan')->truncate();
    	DB::connection('pgsql2')->table('master_urusan')->insert($datas);
    	DB::table('master_urusan')->insert($datas);


        $datas=DB::connection('pgsql2')->table('ms_sub_urusan_23')->get();
        

        $datas=json_decode($datas,true);

        $tbd=DB::connection('pgsql2')->table('master_sub_urusan')->truncate();
        $tbd=DB::table('master_sub_urusan')->truncate();
        ...
        $r['sipd']=static::sipd();
        $r['daerah']=static::lokasi();
        $r['user_role']=static::user_role();
        $r['nomenklatur']=static::sipd_kegiatan();


        return $r;


    	return 'data table urusan_23 success';
    ...
    public static function sipd(){

    	$table='perumahan_kegiatan_2';
    	$kode_urusan=4;
    	$tahun=2020;

    	$p_datas=DB::connection('pgsql2')->select("select CASE WHEN RIGHT(kodedaerah,2)='00' THEN LEFT(kodedaerah,2) ELSE kodedaerah END as kode_daerah,nama_program as label_program,kodeprogram as kode_program,CASE WHEN RIGHT(kodedaerah,2)='00' THEN 1 ELSE 0 END   as tag_provinsi,".$kode_urusan." as id_urusan,".$tahun." as tahun  from ".$table." group by kodeprogram,nama_program,kodedaerah");

    	$p_datas=json_encode($p_datas,true);
    	$p_datas=json_decode($p_datas,true);

    	DB::connection('pgsql2')->table('program')->truncate();

    	DB::connection('pgsql2')->table('program')->insert($p_datas);
    ...
    public static function sipd_kegiatan(){

        // $table='perumahan_kegiatan_2';
        // $kode_urusan=4;
        // $tahun=2020;
        // $query="select CASE WHEN RIGHT(kodedaerah,2)='00' THEN LEFT(kodedaerah,2) ELSE kodedaerah END as kode_daerah,nama_program as label_program,kodeprogram as kode_program,CASE WHEN RIGHT(kodedaerah,2)='00' THEN 1 ELSE 0 END   as tag_provinsi,".$kode_urusan." as id_urusan,".$tahun." as tahun  from ".$table." group by kodeprogram,nama_program,kodedaerah";

        // dd($query);
        // $p_datas=DB::connection('pgsql2')->select($query);
        ...
        // return 'data table program success';


        $data=[];
        DB::connection('pgsql2')->table('ms_nomenklatur_kabkota')->select(['id','urusan','bidang_urusan','program','kegiatan','sub_kegiatan','nomenklatur','kode','jenis'])->orderBy('id','ASC')->chunk(200,function($nomenklatur) use (&$data) {
            $data[]=$nomenklatur->toArray();
        });
        ...
        DB::table('master_nomenklatur_kabkota')->truncate();
        DB::connection('pgsql2')->table('master_nomenklatur_kabkota')->truncate();

        foreach ($data as $key => $value) {
            DB::table('master_nomenklatur_kabkota')->insert($value);
            DB::connection('pgsql2')->table('master_nomenklatur_kabkota')->insert($value);

        }


        $data=[];
        DB::connection('pgsql2')->table('ms_nomenklatur_provinsi')->select(['id','urusan','bidang_urusan','program','kegiatan','sub_kegiatan','nomenklatur','kode','jenis'])->orderBy('id','ASC')->chunk(200,function($nomenklatur) use (&$data) {
            $data[]=$nomenklatur->toArray();
        });
        ...
        DB::table('master_nomenklatur_provinsi')->truncate();
        DB::connection('pgsql2')->table('master_nomenklatur_provinsi')->truncate();

        foreach ($data as $key => $value) {
            DB::table('master_nomenklatur_provinsi')->insert($value);
            DB::connection('pgsql2')->table('master_nomenklatur_provinsi')->insert($value);
